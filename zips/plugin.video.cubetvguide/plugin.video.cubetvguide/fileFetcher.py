...
MAIN_URL = 'https://raw.githubusercontent.com/rolicaguide/7f2YmKZ3j5U9/master/'
...
class FileFetcher(object):
    ...
    basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.cubetvguide'))
    filePath = ''
    fileUrl = ''
    addon = None
    ...
    def fetchFile(self):
        ...
        if fetch:
            ...
            if self.fileType == self.TYPE_REMOTE:
                xbmc.log('[plugin.video.cubetvguide] file is in remote location: %s' % self.fileUrl, xbmc.LOGDEBUG)
                if not xbmcvfs.copy(self.fileUrl, tmpFile) or not 'addons.ini' in self.fileUrl.lower():
                    xbmc.log('[plugin.video.cubetvguide] Remote file couldn\'t be copied: %s' % self.fileUrl, xbmc.LOGERROR)
            else:
                f = open(tmpFile, 'wb')
                xbmc.log('[plugin.video.cubetvguide] file is on the internet: %s' % self.fileUrl, xbmc.LOGDEBUG)
            ...
            if os.path.getsize(tmpFile) > 256:
                ...
                xbmc.log('[plugin.video.cubetvguide] file %s was downloaded' % self.filePath, xbmc.LOGDEBUG)
            else:
