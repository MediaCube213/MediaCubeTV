...
if __name__ == '__main__':
    try:
        ...
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(plugin.video.cubetvguide)")
        
        if ADDON.getSetting('background.service') == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[plugin.video.cubetvguide] Background service started...", xbmc.LOGDEBUG)
            Service()
            ...
            if interval == 0:
                ...
                waitTime = 172800  # 24hrs
            while not monitor.abortRequested():
                # Sleep/wait for specified time
                xbmc.log("[plugin.video.cubetvguide] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                if monitor.waitForAbort(waitTime):
                    # Abort was requested while waiting. We should exit
                    break
                xbmc.log("[plugin.video.cubetvguide] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                
    except source.SourceNotConfiguredException:
        pass  # ignore
    except Exception, ex:
        xbmc.log('[plugin.video.cubetvguide] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)


ADDON = xbmcaddon.Addon('plugin.video.cubetvguide')
if ADDON.getSetting('enable.osd') == "true":
    ADDON.setSetting('enable.osd','false')
