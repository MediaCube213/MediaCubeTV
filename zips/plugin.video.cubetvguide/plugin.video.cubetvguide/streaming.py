...
class StreamsService(object):
    def __init__(self, addon):
        if int(addon.getSetting('addons.ini.type')) == 0:
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.cubetvguide', 'addons.ini'))
            xbmc.log('[plugin.video.cubetvguide] FTV addons.ini is used', xbmc.LOGDEBUG)
        else:
            customFile = str(addon.getSetting('addons.ini.file'))
            if os.path.exists(customFile):
                path = customFile
            else:
                path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.cubetvguide', customFile.split('/')[-1]))
            xbmc.log('[plugin.video.cubetvguide] Custom addons.ini is used: %s' % path, xbmc.LOGDEBUG)
    ...
    def detectStream(self, channel):
        ...
        for label, stream in favourites:
            if label.lower().replace(' ','') == channel.title.lower().replace(' ',''):
                return stream
        ...
        for id in self.getAddons():
            ...
            for (label, stream) in self.getAddonStreams(id):
                ...
                if label.lower().replace(' ','').replace(' ','') == channel.title.lower().replace(' ',''):
