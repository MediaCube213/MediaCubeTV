...
try:
   GUIDEPLUGIN='plugin.video.cubetvguide'
   GUIDEADDON = xbmcaddon.Addon(id=GUIDEPLUGIN)
   SETTO=xbmc.translatePath(os.path.join('special://home', 'addons', 'plugin.video.cubetv','addons.ini'))
   GUIDEADDON.setSetting(id='addons.ini.file', value=SETTO)
except:pass
...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[plugin.video.cubetvguide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[plugin.video.cubetvguide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[plugin.video.cubetvguide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[plugin.video.cubetvguide] Updating caches...', xbmc.LOGDEBUG)
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.cubetvguide'))
    KEY = 'xmltv'
    INI_TYPE_FTV = 0
    INI_TYPE_CUSTOM = 1
    INI_FILE = 'addon.ini'
    ...
    def __init__(self, addon):
        ...
        if self.logoSource == XMLTVSource.LOGO_SOURCE_FTV:
            self.logoFolder = 'https://raw.githubusercontent.com/rolicaguide/7f2YmKZ3j5U9/master/logos/'
        else:
        ...
        if self.xmltvType == gType.CUSTOM_FILE_ID:
            ...
            if os.path.exists(customFile):
                ...
                xbmc.log('[plugin.video.cubetvguide] Use local file: %s' % customFile, xbmc.LOGDEBUG)
                self.xmltvFile = customFile
            else:
                # Probably a remote file
                xbmc.log('[plugin.video.cubetvguide] Use remote file: %s' % customFile, xbmc.LOGDEBUG)
        ...
        #self.updateLocalFile(xbmc.translatePath(os.path.join('special://home', 'addons', 'plugin.video.cubetv','addons.ini')), addon, True)
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            try:
                ...
                xbmc.log('[plugin.video.cubetvguide] strptime error with this date: %s' % dateString, xbmc.LOGDEBUG)
